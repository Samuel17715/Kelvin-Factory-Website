<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-EVSTQN3/azprG1Anm3QDgpJLIm9Nao0Yz1ztcQTwFspd3yD65VohhpuuCOmLASjC" crossorigin="anonymous">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.1.1/animate.min.css" />
    <link rel="stylesheet" href="assets/css/style.css" />
    <title>Forgot Password -Kelvin Factory</title> 
</head>
<body>
    <?php require_once('main-nav.php'); ?>

    <section class="userRegistrationFormContainer flex-center">
        <div class="userRegistrationForm">
            <div class="forgotPasswordDiv">
                <h5 class="text-center mb-4">Forgot Password?</h5>

                <div class="materialFormInput emailAddressInput">
                    <input type="text" class="emailAddress" placeholder="Email Address">
                    <span class="inputFieldInfo">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19.5099 5.85L13.5699 2.42C12.5999 1.86 11.3999 1.86 10.4199 2.42L4.48992 5.85C3.51992 6.41 2.91992 7.45 2.91992 8.58V15.42C2.91992 16.54 3.51992 17.58 4.48992 18.15L10.4299 21.58C11.3999 22.14 12.5999 22.14 13.5799 21.58L19.5199 18.15C20.4899 17.59 21.0899 16.55 21.0899 15.42V8.58C21.0799 7.45 20.4799 6.42 19.5099 5.85ZM11.2499 7.75C11.2499 7.34 11.5899 7 11.9999 7C12.4099 7 12.7499 7.34 12.7499 7.75V13C12.7499 13.41 12.4099 13.75 11.9999 13.75C11.5899 13.75 11.2499 13.41 11.2499 13V7.75ZM12.9199 16.63C12.8699 16.75 12.7999 16.86 12.7099 16.96C12.5199 17.15 12.2699 17.25 11.9999 17.25C11.8699 17.25 11.7399 17.22 11.6199 17.17C11.4899 17.12 11.3899 17.05 11.2899 16.96C11.1999 16.86 11.1299 16.75 11.0699 16.63C11.0199 16.51 10.9999 16.38 10.9999 16.25C10.9999 15.99 11.0999 15.73 11.2899 15.54C11.3899 15.45 11.4899 15.38 11.6199 15.33C11.9899 15.17 12.4299 15.26 12.7099 15.54C12.7999 15.64 12.8699 15.74 12.9199 15.87C12.9699 15.99 12.9999 16.12 12.9999 16.25C12.9999 16.38 12.9699 16.51 12.9199 16.63Z"></path></svg>
                        <span></span>
                    </span>
                </div>

                <div class="materialFormInput">
                    <input type="submit" class="" value="Send Reset Password">
                </div>

                <div class="materialFormSecButton">
                    <a href="create-account.html">New user? <strong>Create Account</strong></a>
                </div>
            </div>
        </div>
    </section>

    <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.0.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-MrcW6ZMFYlzcLA8Nl+NtUVF0sA7MsXsP1UyJoMp4YLEuNSfAP+JcXn/tWtIaxVXM" crossorigin="anonymous"></script>
    <script src="https://kelvinshotzz.com/wp-content/plugins/booking/js/jquery.min.js"></script>
    <script src="assets/js/form-validation.js"></script>
    <script src="assets/js/inner-pages-function.js"></script>
    <script src="assets/js/custom-js.js"></script>
    <script>
        $(document).ready(function() {
            function forgotPasswordCallback(response) {
                switch (response[0]){
                    case 100:
                        console.log('Change Password Email sent');
                        inputValidationFunc('emailAddressInput', '<span class="success">Change Password Link sent to your email</span>', 'open');
                        break;
                    case 101:
                        inputValidationFunc('emailAddressInput', 'This email does not exists. <a href="create-account.html" style="color: #000;"><strong>Create an account here</strong></a>', 'open');
                        break;
                    case 104:
                        console.log('Error! Please try again');
                        break;
                    case 105:
                        console.log('Unable to connect to database');
                }
                $('.forgotPasswordDiv input[type="submit"]').val('Send Reset Password Again');
            }

            // Submit Function (Check input Validation for all and Submit)
            $('.forgotPasswordDiv input[type="submit"]').click(function(event) {
                if( forgotPasswordInputValidationLogin($('.forgotPasswordDiv .emailAddress').val()) ){
                    $(this).val('Sending Verfication Link...');
                    let formData = {
                        email: $('.forgotPasswordDiv .emailAddress').val(),
                        requestType: 'forgetPassword'
                    }
                    callAjaxFunc(formData, forgotPasswordCallback);
                } else {
                }
            })
        });
    </script>
    <script>
        $('.materialNavSection .toggleButton').on('click', function(){
            $('.materialNavSection.mobile nav.dropdown').toggleClass('active');
        });
    </script>
</body>
</html>